...
main() {
  ...
  possible_release_types="release feature bugfix alpha beta rc"

  if [[ ! ${possible_release_types[*]} =~ ${release_type} ]]; then
    echo "valid argument: [ ${possible_release_types[*]} ]"; exit 1
  fi

  release=0; minor=0; patch=0; pre=""; preversion=0
  ...
  next_version="${release}.${minor}.${patch}${pre}"
